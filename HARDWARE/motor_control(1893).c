...
UNS8 len = 0x08;
UNS8 rtr_my =0x00;
u8 encoder_valueleft[4];
u8 encoder_valueright[4];
u8 velocity_valueleft[4],velocity_valueright[4];
u8 current_valueleft[2],current_valueright[2];
u8 status_word_left[2];
u8 status_word_right[2];
u8 stopflag=0;
u8 startcmdstatus=0;
 ...
 void StatusWordAnalysis(void)
{
	...
	if(status_word_left[1]&(0x01<<3))     //判断是否出现FAULT
		MotionHalts();
    if(status_word_right[1]&(0x01<<3))     //判断是否出现FAULT
		MotionHalts();
}
 ...
 void Get_Swerve_Speed_PS2(void)
{
	...
	{
        LED0=!LED0;
		delay_ms(50);	 //延时是必要的，理论上至少是50ms，这里包含了其他延时故设置为10ms以保证延时>50ms
 ...
 * 创建日期  : 2018年11月21日
 * 函数功能  : 设置左右电机速度
 * 输入参数  : float leftvalue   上位机命令中的左轮速度
               float rightvalue  上位机命令中的右轮速度
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
 * 其    它  : 
...
u8 Set_Speed(float leftvalue,float rightvalue)
{
    ...
    /* 上位机发送电机转速2018.11.21*/
#if 1	
     if(leftvalue>3000.0f||rightvalue>3000.0f)
    ...
    /*上位机发送整车速度2018.11.21*/
#if 0
/* delete begin by jishubao, 2018-11-21, Mantis号:1*/
...
/*****************************************************************************
 * 函 数 名  : Get_Encoder_Value
 * 负 责 人  : jishubao
 * 创建日期  : 2018年11月21日
 * 函数功能  : 分析驱动器报文得到左右轮编码器值
 * 输入参数  : void  无
 * 输出参数  : 无
 * 返 回 值  : void
 * 调用关系  : 
 ...
 * 负 责 人  : jishubao
 * 创建日期  : 2018年11月21日
 * 函数功能  : 将下位机数据通过串口上传至上位机
 * 输入参数  : u8 msgid  控制消息类型
 * 输出参数  : 无
 * 返 回 值  : 
 * 调用关系  : 
 * 其    它  : 
 * 修改日志  :    20181121 by jishubao, 删除上传完编码器值后将编码-
                                      器缓存清空，清空导致编码-
                                      器未及时更新时上传的编码-
                                      器值为0，
...
void Date_Up_Load(u8 msgid)
{
	...
	u16 crccheck=0;
		USART_TX_BUF[0]=0x55;
		USART_TX_BUF[1]=0xaa;
		USART_TX_BUF[2]=msgid;  	//type
		USART_TX_BUF[3]=0X00;		//length
...
//		for(j=0;j<USART_TX_BUF[2]+3;j++)
//		{
//			USART_TX_BUF[USART_TX_BUF[2]+3]^=USART_TX_BUF[j];
//		}
//		USART_TX_BUF[20]=USART_TX_BUF[0]^USART_TX_BUF[1]^USART_TX_BUF[2]^USART_TX_BUF[3]^USART_TX_BUF[4]^USART_TX_BUF[5]^USART_TX_BUF[6]^USART_TX_BUF[7]^USART_TX_BUF[8]^USART_TX_BUF[9]^USART_TX_BUF[10]^USART_TX_BUF[11]^USART_TX_BUF[12];
	}
	#if 0
	else if(msgid==0x02)  //上传电池信息
	{
		USART_TX_BUF[2]=0x0c;  //数据长度为12字节
		
		USART_TX_BUF[5]=VoltageValue[0];
		USART_TX_BUF[6]=VoltageValue[1];
		
		USART_TX_BUF[7]=CurrentValue[0];
		USART_TX_BUF[8]=CurrentValue[1];
		
		USART_TX_BUF[9]=NumberDischarges[0];
		USART_TX_BUF[10]=NumberDischarges[1];
		
		USART_TX_BUF[11]=ChargingTimes[0];
		USART_TX_BUF[12]=ChargingTimes[1];
		
		USART_TX_BUF[13]=BatteryStatus[0];
		USART_TX_BUF[14]=BatteryStatus[1];
		
		USART_TX_BUF[15]=BatteryTemperature;
		
		USART_TX_BUF[16]=USART_TX_BUF[0]^USART_TX_BUF[1]^USART_TX_BUF[2]^USART_TX_BUF[3]^USART_TX_BUF[4]^USART_TX_BUF[5]^USART_TX_BUF[6]^USART_TX_BUF[7]^USART_TX_BUF[8]^USART_TX_BUF[9]^USART_TX_BUF[10]^USART_TX_BUF[11]^USART_TX_BUF[12]^USART_TX_BUF[13]^USART_TX_BUF[14]^USART_TX_BUF[15];
	
	}
	else if(msgid==RESET_STATUS_MSGID_CMD) //初始化下位机硬件
	{
		
	}
	else if(msgid==MOTION_VELOCITY) //上传电机速度执行反馈信息uninteger16
	{
		USART_TX_BUF[2]=0x09;  //数据长度
		USART_TX_BUF[5]=velocity_valueleft[0];
		USART_TX_BUF[6]=velocity_valueleft[1];
		USART_TX_BUF[7]=velocity_valueleft[2];
		USART_TX_BUF[8]=velocity_valueleft[3];
        USART_TX_BUF[9]=velocity_valueright[0];
		USART_TX_BUF[10]=velocity_valueright[1];
		USART_TX_BUF[11]=velocity_valueright[2];
		USART_TX_BUF[12]=velocity_valueright[3];
		USART_TX_BUF[13]=0;
		for(j=0;j<USART_TX_BUF[2]+4;j++)
		{
			USART_TX_BUF[USART_TX_BUF[2]+4]^=USART_TX_BUF[j];
		}
//		USART_TX_BUF[9]=USART_TX_BUF[0]^USART_TX_BUF[1]^USART_TX_BUF[2]^USART_TX_BUF[3]^USART_TX_BUF[4]^USART_TX_BUF[5]^USART_TX_BUF[6]^USART_TX_BUF[7]^USART_TX_BUF[8];
	}
	#endif
...
//	  USART_TX_BUF[USART_TX_BUF[2]+4] =0x0a;        //串口换行调试用
	if(USART_TX_BUF[3])
	{
		for(j=0;j<USART_TX_BUF[3]+6;j++)//循环发送数据
		{
			while(USART_GetFlagStatus(USART1,USART_FLAG_TC)==RESET);//循环发送，知道发送完毕
			USART_SendData(USART1,(uint8_t)USART_TX_BUF[j]);   
		}
	}
	memset(USART_TX_BUF,0,USART_TX_BUF[3]+6);
	/* delete begin by jishubao, 2018-11-21, Mantis号:1 
	原因: 删除上传完编码器值后将编码               器缓存清空，清空导致编码            器未及时更新时上传的编码            器值为0 */
...
void Motion_Status(void)
{
    ...
    if(RxMSG.data[0]==data_cmd_MotionStatus[0]&&RxMSG.data[1]==data_cmd_MotionStatus[1]&&RxMSG.data[2]==data_cmd_MotionStatus[2]&&RxMSG.data[3]==data_cmd_MotionStatus[3])//判断接收到的报文是否为编码器值报文
	{
